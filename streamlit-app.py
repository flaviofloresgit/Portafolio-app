...
def mes_inmediato_anterior(fecha):
    """Mes calendario inmediatamente anterior a la fecha dada (Regla 3.15.12)."""
    dt = pd.to_datetime(fecha)
    dt_ant = dt - pd.DateOffset(months=1)
    return int(dt_ant.year), int(dt_ant.month)

def obtener_inpc_por_fecha(fecha_str):
    """INPC del mes inmediato anterior a la fecha de la operación (equivalente a TC DOF pero desde Supabase)."""
    anio, mes = mes_inmediato_anterior(fecha_str)
    return obtener_inpc(anio, mes)

def obtener_inpc_por_fecha_seguro(fecha_str):
    try:
        return obtener_inpc_por_fecha(fecha_str)
    except Exception as e:
        st.warning(f"No se obtuvo INPC para {fecha_str}: {e}")
        return None

def calcular_factor_inpc_31512(fecha_mes_inicio, fecha_mes_fin):
    """Factor INPC Regla 3.15.12: mes anterior a fin / mes anterior a inicio."""
    anio_ini, mes_ini = mes_inmediato_anterior(fecha_mes_inicio)
    anio_fin, mes_fin = mes_inmediato_anterior(fecha_mes_fin)
    if anio_ini == anio_fin and mes_ini == mes_fin:
        return 1.00000
    inpc_ini = obtener_inpc(anio_ini, mes_ini)
    inpc_fin = obtener_inpc(anio_fin, mes_fin)
    return max(1.0, round(inpc_fin / inpc_ini, 5))

# Calcular el factor INPC (tab FIFO — PEPS por lote)
...
tab_tablas, tab_dividendos, tab_reporte, tab_promedio = st.tabs(["📜 Registros", "💰 Reporte de Dividendos", "📈 Reporte Fiscal (FIFO)", "📊 Reporte Costo Promedio (Art. 129)"])
...
# =============================================
# Pestaña 4: Reporte Costo Promedio (Art. 129).
# =============================================
with tab_promedio:
    st.subheader("Resumen Fiscal — Costo Promedio (Art. 129 LISR)")

    if "df_promedio" not in st.session_state:
        st.session_state["df_promedio"] = None
    if "promedio_anio" not in st.session_state:
        st.session_state["promedio_anio"] = None
    if "promedio_broker" not in st.session_state:
        st.session_state["promedio_broker"] = None
    
    ventas_tab_prom = fetch_all_rows("ventas", order_col="fecha_hora", desc=False)
    with st.expander("Filtros para reporte"):
        col_p1, col_p2 = st.columns(2)
        with col_p1:
            broker_filtro_prom = st.selectbox("Filtrar por Broker", ["Todos"] + BROKERS, key="f_promedio_broker")
        with col_p2:
            df_ventas_anio_prom = pd.DataFrame(ventas_tab_prom or [])
            if df_ventas_anio_prom.empty or "fecha" not in df_ventas_anio_prom.columns:
                anios_disponibles_prom = ["Todos"]
            else:
                df_ventas_anio_prom["anio"] = df_ventas_anio_prom["fecha"].astype(str).str[:4]
                anios_disponibles_prom = ["Todos"] + sorted(df_ventas_anio_prom["anio"].unique(), reverse=True)
                        
            anio_filtro_prom = st.selectbox("Ejercicio fiscal (Año de venta)", anios_disponibles_prom, key="f_promedio_anio")
    
    calcular_promedio = st.button("Calcular Impuestos (Costo Promedio)")

    if calcular_promedio:
        q_compras_prom = fetch_all_rows("compras", order_col="fecha_hora", desc=False)
        q_ventas_prom = ventas_tab_prom

        if not q_compras_prom or not q_ventas_prom:
            st.session_state["df_promedio"] = None
            st.warning("Necesitas tener al menos una compra y una venta registrada para ejecutar el cálculo.")
        else:
            df_compras_prom = pd.DataFrame(q_compras_prom).sort_values(["fecha_hora", "id"]).reset_index(drop=True)
            df_ventas_prom = pd.DataFrame(q_ventas_prom).sort_values(["fecha_hora", "id"]).reset_index(drop=True)

            for col in ["cantidad", "precio_usd", "comision_usd"]:
                df_compras_prom[col] = pd.to_numeric(df_compras_prom[col])
                df_ventas_prom[col] = pd.to_numeric(df_ventas_prom[col])

            resultados_promedio = []
            grupos_prom = df_ventas_prom[["broker", "ticker"]].drop_duplicates().to_numpy()
            calculo_fallido = False

            for b, t in grupos_prom:
                compras_t = df_compras_prom[(df_compras_prom["broker"] == b) & (df_compras_prom["ticker"] == t)].copy()
                ventas_t = df_ventas_prom[(df_ventas_prom["broker"] == b) & (df_ventas_prom["ticker"] == t)].copy()

                ops = []
                for _, c in compras_t.iterrows():
                    ops.append({"tipo": "compra", "fecha_hora": c["fecha_hora"], "id": c["id"], "data": c.to_dict()})
                for _, v in ventas_t.iterrows():
                    ops.append({"tipo": "venta", "fecha_hora": v["fecha_hora"], "id": v["id"], "data": v.to_dict()})
                ops.sort(key=lambda x: (x["fecha_hora"], x["id"]))

                estado = {
                    "qty": 0.0,
                    "promedio_mxn": 0.0,
                    "costo_total_mxn": 0.0,
                    "fecha_ultima_act_inpc": None,
                }
                ...
                for op in ops:
                    if op["tipo"] == "compra":
                        c = op["data"]
                        cant_c = float(c["cantidad"])
                        costo_usd = (cant_c * float(c["precio_usd"])) + float(c.get("comision_usd", 0.0))
                        tc_compra = obtener_tc_dof_seguro(c["fecha"])
                        if tc_compra is None:
                            st.error(f"Sin TC DOF para compra de {t} ({c['fecha']}).")
                            calculo_fallido = True
                            break

                        costo_compra_mxn = costo_usd * tc_compra

                        if estado["qty"] <= 0:
                            estado["promedio_mxn"] = costo_compra_mxn / cant_c
                            estado["costo_total_mxn"] = costo_compra_mxn
                            estado["qty"] = cant_c
                            estado["fecha_ultima_act_inpc"] = c["fecha"]
                        else:
                            try:
                                factor_compra = calcular_factor_inpc_31512(estado["fecha_ultima_act_inpc"], c["fecha"])
                            except Exception as e:
                                st.error(f"Sin INPC para compra de {t} ({c['fecha']}): {e}")
                                calculo_fallido = True
                                break

                            promedio_actualizado = estado["promedio_mxn"] * factor_compra
                            costo_viejo = estado["qty"] * promedio_actualizado
                            estado["qty"] += cant_c
                            estado["costo_total_mxn"] = costo_viejo + costo_compra_mxn
                            estado["promedio_mxn"] = estado["costo_total_mxn"] / estado["qty"]
                            estado["fecha_ultima_act_inpc"] = c["fecha"]

                    else:
                        v = op["data"]
                        cant_v = float(v["cantidad"])
                        precio_v = float(v["precio_usd"])
                        comision_v = float(v.get("comision_usd", 0.0))
                        fecha_v = v["fecha"]

                        if round(cant_v, 9) > round(estado["qty"], 9):
                            st.error(f"⚠️ Error: Venta de {cant_v} acciones de {t} sin compras suficientes (disponibles: {estado['qty']}).")
                            calculo_fallido = True
                            break

                        if estado["qty"] <= 0 or estado["fecha_ultima_act_inpc"] is None:
                            st.error(f"⚠️ Error: Venta de {t} sin compras previas registradas.")
                            calculo_fallido = True
                            break

                        tc_venta = obtener_tc_dof_seguro(fecha_v)
                        if tc_venta is None:
                            st.error(f"Sin TC DOF para venta de {t} ({fecha_v}).")
                            calculo_fallido = True
                            break

                        try:
                            factor_venta = calcular_factor_inpc_31512(estado["fecha_ultima_act_inpc"], fecha_v)
                        except Exception as e:
                            st.error(f"Sin INPC para venta de {t} ({fecha_v}): {e}")
                            calculo_fallido = True
                            break

                        promedio_pre_venta = estado["promedio_mxn"]
                        costo_por_accion_ajustado = promedio_pre_venta * factor_venta
                        costo_venta_mxn = cant_v * costo_por_accion_ajustado

                        ingreso_venta_usd = (cant_v * precio_v) - comision_v
                        ingreso_venta_mxn = ingreso_venta_usd * tc_venta
                        ganancia_mxn = ingreso_venta_mxn - costo_venta_mxn
                        ...
                        resultados_promedio.append({
                            ...
                            "Cantidad": cant_v,
                            "Fecha Venta": fecha_v,
                            "Costo Promedio (MXN)": promedio_pre_venta,
                            "Fecha Ref INPC": estado["fecha_ultima_act_inpc"],
                            "Factor INPC": factor_venta,
                            "Costo Ajustado (MXN)": costo_venta_mxn,
                            "Precio Venta (USD)": precio_v,
                            "Comision Venta (USD)": comision_v,
                            "Ingreso Venta (USD)": ingreso_venta_usd,
                            "TC DOF Venta": tc_venta,
                            "Ingreso Venta (MXN)": ingreso_venta_mxn,
                            "Ganancia/Perdida (MXN)": ganancia_mxn,
                        ...
                        estado["costo_total_mxn"] -= cant_v * promedio_pre_venta
                        estado["qty"] = round(estado["qty"] - cant_v, 9)

                        if estado["qty"] <= 0:
                            estado["qty"] = 0.0
                            estado["promedio_mxn"] = 0.0
                            estado["costo_total_mxn"] = 0.0
                            estado["fecha_ultima_act_inpc"] = None
            ...
            if calculo_fallido:
                st.session_state["df_promedio"] = None
            elif resultados_promedio:
                df_res_prom = pd.DataFrame(resultados_promedio)

                if broker_filtro_prom != "Todos":
                    df_res_prom = df_res_prom[df_res_prom["Broker"] == broker_filtro_prom]
                if anio_filtro_prom != "Todos":
                    df_res_prom = df_res_prom[df_res_prom["Fecha Venta"].str.startswith(str(anio_filtro_prom))]

                st.session_state["df_promedio"] = df_res_prom
                st.session_state["promedio_anio"] = anio_filtro_prom
                st.session_state["promedio_broker"] = broker_filtro_prom

                if df_res_prom.empty:
                    st.info("No hay ventas registradas para los filtros seleccionados.")
                else:
                    st.toast("Cálculo Costo Promedio realizado con éxito", icon=":material/check_circle:", duration=5)
            else:
                st.session_state["df_promedio"] = None
                st.warning("No se generó ninguna fila del cálculo. Revisa los errores de TC DOF o INPC.")

    df_res_prom = st.session_state["df_promedio"]
    if df_res_prom is not None and not df_res_prom.empty:
        anio_mostrar_prom = st.session_state.get("promedio_anio", "")
        broker_mostrar_prom = st.session_state.get("promedio_broker", "")

        tot_ingreso_usd_prom = round(df_res_prom["Ingreso Venta (USD)"].sum(), 2)
        tot_ingreso_mxn_prom = round(df_res_prom["Ingreso Venta (MXN)"].sum(), 2)
        tot_costo_mxn_prom = round(df_res_prom["Costo Ajustado (MXN)"].sum(), 2)
        tot_ganancia_mxn_prom = round(df_res_prom["Ganancia/Perdida (MXN)"].sum(), 2)
        tot_isr_prom = round(max(0.0, tot_ganancia_mxn_prom * 0.10), 2)

        cp1, cp2, cp3, cp4 = st.columns(4)
        cp1.metric(label="Ingresos Totales", value=f"${tot_ingreso_mxn_prom:,.2f} MXN", delta=f"${tot_ingreso_usd_prom:,.2f} USD", delta_color="off")
        cp2.metric(label="Costo Ajustado INPC", value=f"${tot_costo_mxn_prom:,.2f} MXN", delta_color="off")
        cp3.metric(label="Ganancia/Pérdida Neta", value=f"{tot_ganancia_mxn_prom:,.2f} MXN", delta_color="normal")
        cp4.metric(label="ISR Estimado (10%)", value=f"{tot_isr_prom:,.2f} MXN")

        buffer_excel_prom = io.BytesIO()
        with pd.ExcelWriter(buffer_excel_prom, engine="openpyxl") as writer:
            df_res_prom.to_excel(writer, sheet_name=f"Promedio {anio_mostrar_prom}", index=False)
            worksheet_prom = writer.sheets[f"Promedio {anio_mostrar_prom}"]
            worksheet_prom.views.sheetView[0].showGridLines = True
            for i, col in enumerate(worksheet_prom.columns, start=1):
                max_len = max(len(str(cell.value or "")) for cell in col)
                col_letter = get_column_letter(i)
                worksheet_prom.column_dimensions[col_letter].width = max(max_len + 3, 12)

        st.download_button(
            label="📥 Descargar Papel de Trabajo (Excel)",
            data=buffer_excel_prom.getvalue(),
            file_name=f"Papel_de_Trabajo_Promedio_{broker_mostrar_prom}_{anio_mostrar_prom}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="btn_descarga_excel_promedio",
        )

        st.divider()
        st.subheader(f"Desglose Fiscal de Ventas ({anio_mostrar_prom})")
        st.dataframe(
            df_res_prom,
            width="stretch",
            column_config={
                "Cantidad": st.column_config.NumberColumn(format="%.9f"),
                "Costo Promedio (MXN)": st.column_config.NumberColumn(format="$%.2f"),
                "Factor INPC": st.column_config.NumberColumn(format="%.5f"),
                "Costo Ajustado (MXN)": st.column_config.NumberColumn(format="$%.2f"),
                "Precio Venta (USD)": st.column_config.NumberColumn(format="%.2f"),
                "Comision Venta (USD)": st.column_config.NumberColumn(format="%.2f"),
